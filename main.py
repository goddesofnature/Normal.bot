...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("working")
        await self.highrise.walk_to(Position(0.5 , 0.0 , 6.5 , "FrontRight"))
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.lower().startswith("!tipall ") and user.username == "goddesofnature":
              parts = message.split(" ")
              if len(parts) != 2:
        ...
        if message.lower().startswith("!tipme ") and user.username== "goddesofnature":
                try:
        ...
        if        message.startswith("/tele") or              message.startswith("/tp") or              message.startswith("/fly") or     message.startswith("!tele") or      message.startswith("!tp") or     message.startswith("!fly"):
          if user.username == "goddesofnature":            await self.teleporter(message)
        ...
        if              message.startswith("Fairyfloat All") or                              message.startswith("/emote all fairyfloat") or       message.startswith("!emote all fairyfloat"):
          if user.username == "goddesofnature":
